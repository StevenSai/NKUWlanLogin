...
//        Log.e("wlan", String.format("status check before login:%s,%s,%s",tmp.status,uid,psw));
        ...
        Request request = new Request.Builder().post(requestBody).url(host + login_path).build();
        try {
    ...
    public static boolean checkInternet() {
        ...
        return result.equals("hello future");
    }
    ...
    public static NetworkInfo logout() {
        ...
        for (int i = 0; i < 3; i++) {
            Request request = new Request.Builder().url(host + logout_path).build();
            try {
